...
export default function PlannerPage() {
  ...
  // ==================== AI apply handler ====================
  // ✅ يدعم المهام الأساسية + الأعمال الجانبية معاً
  const handleApplyAIDraft = useCallback(
    (draft: any) => {
      // 1) العنوان
      if (draft.title) setTitle(draft.title)

      // 2) التاريخ (تحقق من الصيغة)
      if (draft.date && /^\d{4}-\d{2}-\d{2}$/.test(draft.date)) {
        setDate(draft.date)
      }

      // 3) وقت البدء (تحقق من الصيغة)
      ...
      if (Array.isArray(draft.sideTasks) && draft.sideTasks.length > 0) {
        ...
        if (newSideTasks.length > 0) {
          setSideTasks(newSideTasks)
        }
      }

      toast.success(
        tr('✨ تم تطبيق اقتراح جَدْوُولْ! راجعه قبل الإنشاء', '✨ Jadwool suggestion applied! Review before creating.')
      )
    },
    [tr]
  )
  ...
  // ==================== Reset form ====================
  const resetForm = () => {
    if (!confirm(tr('هل أنت متأكد من مسح كل البيانات؟', 'Are you sure you want to clear all fields?')))
      return
    setTitle('')
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(error)
      toast.error(
        `${tr('حدث خطأ', 'An error occurred')}: ${error.message || tr('غير معروف')}`
      )
    } finally {
      setGenerating(false)
    }
  ...
  // ==================== Render ====================
  return (
    <div className="p-4 sm:p-6 space-y-6" dir={language === 'ar' ? 'rtl' : 'ltr'}>
      {/* Header */}
      <motion.div
      ...
      >
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm font-['Cairo'] mt-1">
            {tr('أنشئ جدولاً يومياً متكاملاً مع مهامك وصلواتك')}
          </p>
        </div>
        <button
          onClick={resetForm}
          className="self-start md:self-auto flex items-center gap-2 px-4 py-2 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[#D4AF37] hover:border-[#D4AF37]/30 transition-all font-['Cairo'] text-sm"
        >
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={tr('مثال: يوم عمل مكثف')}
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-[var(--text-muted)] bg-white/5 px-2 py-1 rounded-full font-['Cairo']">
                  {tasks.length} {tr('مهام')}
                </span>
              </div>
              <button
                onClick={addTask}
              ...
              >
                <Plus className="w-4 h-4" /> {tr('إضافة مهمة')}
              </button>
            </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-2 rounded-xl bg-[#D4AF37]/10 text-[#D4AF37] hover:bg-[#D4AF37]/20 transition-colors font-['Cairo'] text-sm"
              >
                <Plus className="w-4 h-4" /> {tr('إضافة عمل')}
              </button>
            </div>
            <div className="space-y-3">
              <AnimatePresence>
          ...
          >
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={item.key}>
                  <label className="block text-xs text-[var(--text-secondary)] font-['Cairo'] mb-1">
                    {item.label}
                  </label>
                  <input
                    type="number"
          ...
          >
            ...
            <div className="space-y-2">
              {prayers.map((p, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between p-2.5 rounded-xl bg-[var(--bg-secondary)] border border-[var(--border-color)] hover:border-[#D4AF37]/30 transition-colors"
                >
                  <span className="text-sm text-[var(--text-primary)] font-['Cairo']">
                    {tr(p.name)}
                  </span>
                  <span className="text-sm text-[var(--text-secondary)] font-['Cairo']">
