...
function FAQItem({ q, a, index }: { q: string; a: string; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-bold text-[var(--text-primary)] font-['Cairo']">{tr(q)}</span>
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 max-w-6xl mx-auto" >
      ...
      >
        ...
        {tr(" العودة للوحة التحكم ")}</motion.button>
      ...
      >
        ...
        >
          {tr(" قصة بسيطة بدأت بحاجة شخصية، وتحولت إلى أداة يستفيد منها الجميع ")}</motion.p>
      </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div>
            <h2 className="text-3xl font-bold font-['Amiri'] text-[var(--text-primary)] mb-6">
              {tr(" قصتي مع جَدْوَلِي ")}</h2>
            <div className="space-y-4 text-[var(--text-secondary)] font-['Cairo'] leading-relaxed">
              <p>
                {tr(" لم يبدأ جَدْوَلِي كشركة أو فريق كبير، بل بدأ كحاجة شخصية بحتة. كنت أبحث عن طريقة لتنظيم وقتي بين العمل والدراسة والعبادة، فوجدت أن التطبيقات المتاحة إما معقدة أو تهمل الجانب الروحي. ")}</p>
              <p>
                {tr(" قررت أن أبني أداة بسيطة تناسب احتياجاتي: جدول ذكي يجمع مهامي اليومية مع أوقات الصلاة، ونظام بومودورو يساعدني على التركيز. ")}</p>
              <p>
                {tr(" بعد أن استخدمتها بنفسي ورأيت كيف غيّرت إنتاجيتي، أدركت أن هناك الكثيرين مثلي يحتاجون إلى هذه الأداة. فقررت نشرها ليفيد الجميع. ")}</p>
              <p>
                {tr(" اليوم، جَدْوَلِي متاح للجميع مجانًا، وما زلت أطورها بنفسي بشغف. ")}</p>
            </div>
          </div>
          <motion.div
          ...
          >
            ...
            <p className="text-xl font-['Amiri'] text-[var(--text-primary)] leading-relaxed">
              {tr(" \"صنعتُ هذه الأداة لنفسي أولًا، واليوم أشاركها معك لأنني أعلم أنها ستفيدك كما أفادتني\" ")}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-6 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)]">
            ...
            <p className="text-[var(--text-secondary)] font-['Cairo'] leading-relaxed">
              {tr(" أن تكون جَدْوَلِي الأداة العربية الأولى التي تجمع بين الإنتاجية والروحانية ببساطة وفعالية. ")}</p>
          </div>
          <div className="p-6 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-card)]">
            ...
            <h3 className="text-2xl font-bold font-['Amiri'] text-[var(--text-primary)] mb-3">
              {tr(" رسالتي ")}</h3>
            <p className="text-[var(--text-secondary)] font-['Cairo'] leading-relaxed">
              {tr(" توفير أداة مجانية وسهلة تساعد كل شخص على تنظيم يومه دون تعقيد، مع احترام الجانب الروحي. ")}</p>
          </div>
        </div>
      </motion.section>
      ...
      >
        <h2 className="text-3xl font-bold font-['Amiri'] text-[var(--text-primary)] mb-8 text-center">
          {tr(" لماذا جَدْوَلِي؟ ")}</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((feature, index) => (
            <motion.div
              key={tr(feature.title)}
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
